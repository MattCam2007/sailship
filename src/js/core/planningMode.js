...
import { celestialBodies, updateCelestialPositions, getBodyByName } from '../data/celestialBodies.js';
...
export function getShipPosition() {
    ...
    if (mode === 'FIXED') {
        ...
        if (snapshot.playerShip.coordinateFrame === 'PLANETOCENTRIC') {
            const parentBodyName = snapshot.playerShip.soiState.currentBody;
            const parentBody = getBodyByName(parentBodyName);
            if (parentBody) {
                return {
                    x: parentBody.x + position.x,
                    y: parentBody.y + position.y,
                    z: parentBody.z + position.z
                };
            }
        }
        return position;
 ...
 * NOTE: Uses same unit conventions as orbital.js (mu in km^3/s^2, a in km)
...
function propagateElements(elements, julianDate) {
    ...
    const n = Math.sqrt(elements.mu / a3);
