...
import {
    evaluateCandidate,
    getInterceptThreshold,
    solveQuadraticCrossing
} from './evaluate-trajectory.js';
import { WorkerPool } from '../workers/worker-pool.js';

// Re-export for consumers that import from course-solver.js
export { evaluateCandidate, getInterceptThreshold, solveQuadraticCrossing };
...
// ============================================================================
// CROSSING DETECTION HELPERS - Now in evaluate-trajectory.js
// evaluateCandidate, getInterceptThreshold, solveQuadraticCrossing
// are imported from evaluate-trajectory.js and re-exported above.
// ============================================================================

// ============================================================================
// NOTE: The following functions were extracted to evaluate-trajectory.js:
//   calculateAngularSeparation, solveQuadraticCrossing,
//   findRadiusCrossingsInTrajectory, distance3D,
//   getInterceptThreshold, evaluateCandidate
// ============================================================================
// ============================================================================
...
export async function strategicReconnaissance(ship, target, options = {}, bounds = null) {
    ...
    // Parallel path: batch all probes to worker pool
    if (workerCount > 1) {
        const pool = new WorkerPool(workerCount);
        try {
            const items = probes.map(({ yaw, pitch }) => ({
                yawDeg: Math.max(CONFIG.yawMin, Math.min(CONFIG.yawMax, yaw)),
                pitchDeg: Math.max(CONFIG.pitchMin, Math.min(CONFIG.pitchMax, pitch)),
                options: {
                    startJulianDate: options.startJulianDate,
...
export async function solveCourse(ship, target, options = {}, onProgress = null) {
    ...
    // Ensure startJulianDate is set for all downstream evaluateCandidate calls
    if (!options.startJulianDate) {
        options = { ...options, startJulianDate: getJulianDate() };
    }
...
export async function coarseSweep(ship, target, options = {}, onProgress = null) {
    ...
    if (workerCount > 1) {
        const pool = new WorkerPool(workerCount);
        try {
            const items = candidates.map(({ yaw, pitch }) => ({
                yawDeg: yaw,
                pitchDeg: pitch,
                options: {
                    startJulianDate: options.startJulianDate,
